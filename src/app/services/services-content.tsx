...
export default function ServicesContent() {
  ...
  const shareOnWhatsApp = (service: any) => {
    ...
    const text = `*${appName.toUpperCase()} - DETALHES DO SERVIÇO*
...
_Gerado por ${appName}_`
  ...
  const generatePDF = (service: any) => {
    ...
    doc.text(appName.toUpperCase(), 20, 20)
    
    doc.setFontSize(12)
    doc.text("ORDEM DE SERVIÇO", 20, 30)
          ...
          <DialogContentSimple className="sm:max-w-[700px] w-[95vw] max-h-[90vh] rounded-[3rem] p-0 border-0 shadow-2xl overflow-y-auto">
            {viewingService && (
              <div className="flex flex-col">
                ...
                <div className="p-2 space-y-2">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <div className="space-y-1">
                      ...
                      <div className="p-2 rounded-xl bg-muted/30 border-2 border-transparent">
                        <p className="text-xs font-black">{clients?.find(c => c.id === viewingService.clientId)?.name || "N/A"}</p>
                        <p className="text-[9px] font-bold text-muted-foreground">{clients?.find(c => c.id === viewingService.clientId)?.phone || "Sem telefone"}</p>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <h4 className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-1">
                        <DollarSign className="w-3 h-3 text-emerald-500" /> Financeiro
                      </h4>
                      <div className="p-2 rounded-xl bg-emerald-500/5 border-2 border-emerald-500/10">
                        <p className="text-[8px] font-black uppercase text-emerald-600/60">Valor Cobrado</p>
                        <p className="text-lg font-black text-emerald-600">R$ {Number(viewingService.chargedValue).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <h4 className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-1">
                      <Info className="w-3 h-3 text-blue-500" /> Observações
                    </h4>
                    <div className="p-2 rounded-xl bg-blue-500/5 border-2 border-blue-500/10 min-h-[50px]">
                      <p className="text-[10px] font-medium leading-relaxed italic text-blue-900/70">
                  ...
                  {viewingService.materials && viewingService.materials.length > 0 && (
                    <div className="space-y-1">
                      <h4 className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-1">
                        <Package className="w-3 h-3 text-orange-500" /> Materiais
                      </h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                        {viewingService.materials.map((m: any, idx: number) => {
                          const product = products?.find(p => p.id === m.productId)
                          return (
                            <div key={idx} className="flex items-center gap-2 p-1.5 rounded-lg bg-muted/30 border-2 border-transparent">
                              <div className="p-0.5 bg-orange-500/10 rounded">
                                <Package className="w-2.5 h-2.5 text-orange-500" />
                              </div>
                              <div className="min-w-0">
                                <p className="text-[9px] font-black truncate">{product?.name || "Material"}</p>
                                <p className="text-[7px] font-bold text-muted-foreground uppercase">{m.quantity} {product?.unit || "un"}</p>
                              </div>
                            </div>
                          )
                        })}
                ...
                <div className="p-2 bg-muted/30 border-t flex flex-row gap-2">
                  <Button 
                    onClick={() => shareOnWhatsApp(viewingService)}
                    className="flex-1 h-10 rounded-lg bg-[#25D366] hover:bg-[#20ba5a] text-white font-black text-xs gap-1 shadow-sm shadow-[#25D366]/20"
                  >
                  ...
                  <Button 
                    ...
                    variant="outline"
                    className="flex-1 h-10 rounded-lg border-2 border-primary/20 font-black text-xs gap-1 hover:bg-primary/5"
                  >
                    <FileDown className="w-3.5 h-3.5 text-primary" /> PDF
                  </Button>
                  <Button 
                    onClick={() => setViewingService(null)} 
                    variant="outline"
                    className="flex-1 h-10 rounded-lg border-2 border-destructive/20 hover:bg-destructive/10 hover:text-destructive"
                  >
